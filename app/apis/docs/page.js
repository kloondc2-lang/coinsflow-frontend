...
export const metadata = {
  ...
  description: 'Complete reference for the CoinsFlow Blockchain API. Endpoints, authentication, examples, and error codes.',
};
  ...
  return (
    <tr className="border-t border-white/[0.05]">
      <td className="py-3 pr-4 align-top">
        <code className="text-[12.5px] font-mono text-blue-400">{name}</code>
        {required && <span className="ml-1.5 text-[9px] font-bold uppercase text-red-400/70 tracking-wider">req</span>}
      </td>
      <td className="py-3 pr-4 align-top">
        <span className="text-[11px] font-mono text-[#334155] bg-white/[0.04] px-1.5 py-0.5 rounded">{type}</span>
      </td>
      <td className="py-3 text-[13px] text-[#64748b] align-top">{desc}</td>
    </tr>
  );
}
...
function CodeBlock({ lang = 'bash', children }) {
  return (
    <div className="rounded-lg border border-white/[0.06] overflow-hidden my-4">
      ...
      <pre className="px-4 py-4 bg-[#040c1a] text-[12.5px] font-mono text-[#94a3b8] leading-relaxed overflow-x-auto whitespace-pre">
        <code>{children}</code>
      </pre>
    </div>
  );
...
function EndpointSection({ id, method, path, title, desc, params, requestExample, responseExample }) {
  return (
    <section id={id} className="pt-16 border-t border-white/[0.06]">
          ...
          <div className="rounded-lg border border-white/[0.06] overflow-hidden">
            <table className="w-full text-left">
              ...
              <tbody className="divide-y divide-white/[0.04] bg-[#040c1a]">
                {params.map((p) => (
                  <tr key={p.name} className="px-4">
                    <td className="px-4 py-3 align-top">
                      <code className="text-[12.5px] font-mono text-blue-400">{p.name}</code>
                      {p.required && <span className="ml-1.5 text-[9px] font-bold uppercase text-red-400/70 tracking-wider">req</span>}
                    </td>
                    <td className="px-4 py-3 align-top">
      ...
      <CodeBlock lang="bash">{requestExample}</CodeBlock>

      <h4 className="text-[12px] font-semibold text-[#334155] uppercase tracking-widest mb-1">Response</h4>
      ...
      <div className="max-w-[1280px] mx-auto flex">
        ...
        <aside className="hidden lg:block w-[220px] flex-shrink-0 sticky top-[68px] h-[calc(100vh-68px)] overflow-y-auto py-10 pr-4">
          ...
          <div className="mt-8 px-3">
            <Link href="/apis/dashboard" className="block text-center px-3 py-2 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-[12px] font-semibold transition-colors">
              Get API Key
            </Link>
          </div>
        </aside>
        ...
        <main className="flex-1 min-w-0 px-6 md:px-12 lg:px-10 xl:px-16 py-12">
          ...
          <div className="flex flex-wrap gap-2 mt-4 mb-12">
            {['REST', 'JSON', 'API Key Auth', 'Litecoin'].map((t) => (
          ...
          <section id="getting-started" className="border-t border-white/[0.06] pt-12">
            ...
            <ol className="space-y-3 text-[14px] text-[#64748b]">
            ...
            <CodeBlock lang="bash">{`curl https://api.coinsflow.net/v1/price/ltc \\
  -H "X-API-Key: cf_live_your_key_here"`}</CodeBlock>
          </section>
...
curl https://api.coinsflow.net/v1/address/ltc/LXqvJaXc9xC8... \\
              ...
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-[#071423]">
                    ...
                    <th className="px-4 py-2.5 text-[11px] font-semibold text-[#4a5568] uppercase tracking-wider">Burst</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-white/[0.04] bg-[#040c1a]">
                  <tr>
                    <td className="px-4 py-3 text-[13px] text-[#64748b]">Free</td>
                    <td className="px-4 py-3 text-[13px] text-[#e2e8f0] font-mono">500</td>
                    <td className="px-4 py-3 text-[13px] text-[#e2e8f0] font-mono">10 req/s</td>
                  </tr>
                  <tr>
                    <td className="px-4 py-3 text-[13px] text-[#64748b]">Pro</td>
                    <td className="px-4 py-3 text-[13px] text-[#e2e8f0] font-mono">50,000</td>
                    <td className="px-4 py-3 text-[13px] text-[#e2e8f0] font-mono">100 req/s</td>
                  </tr>
                </tbody>
              </table>
            </div>
            <p className="text-[13.5px] text-[#64748b]">
              When a rate limit is exceeded, the API returns <code className="text-blue-400 font-mono text-[13px] bg-white/[0.04] px-1 rounded">429 Too Many Requests</code>. The response includes a <code className="text-blue-400 font-mono text-[13px] bg-white/[0.04] px-1 rounded">Retry-After</code> header indicating when to retry.
            </p>
          </section>
  ...
  "error": "Invalid or missing API key"
          ...
          <EndpointSection
            ...
            requestExample={`curl https://api.coinsflow.net/v1/address/ltc/LXqvJaXc9xC8UjFEDRTDjzD8bNHzMpBMQJ \\
  ...
  "transactions": [
    {
      ...
      "timestamp": 1714003201
          ...
          <EndpointSection
            ...
            requestExample={`curl https://api.coinsflow.net/v1/tx/ltc/a3f9c12b4e7d... \\
  -H "X-API-Key: cf_live_your_key_here"`}
            responseExample={`{
  "txid": "a3f9c12b4e7d...",
  ...
  "outputs": [
    ...
    { "address": "LXqvJaXc9x...", "value": 0.49995800 }
          ...
          <EndpointSection
            ...
            requestExample={`curl https://api.coinsflow.net/v1/block/ltc/2831047 \\
  -H "X-API-Key: cf_live_your_key_here"`}
            responseExample={`{
  "hash": "0000000000000...",
  "height": 2831047,
  ...
  "transactions": ["a3f9c12b...", "7b2d4f1a..."]
          ...
          <EndpointSection
            ...
            params={[]}
            requestExample={`curl https://api.coinsflow.net/v1/blocks/ltc \\
  -H "X-API-Key: cf_live_your_key_here"`}
            responseExample={`{
  "blocks": [
    {
      ...
      "size": 48291
          ...
          <EndpointSection
            ...
            params={[]}
            requestExample={`curl https://api.coinsflow.net/v1/price/ltc \\
  -H "X-API-Key: cf_live_your_key_here"`}
            responseExample={`{
  "coin": "litecoin",
  "symbol": "LTC",
